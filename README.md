...
Для примера представлен набор из Dockerfile, docker-compose манифеста и скрипта по настройке. Инструкция:

1. Склонировать репозиторий в нужное расположение
2. Перейти в директорию incender_standalone
3. Выполнить chmod +x init.sh
4. Запустить init.sh
5. Заполнить предложенные мастером параметры
